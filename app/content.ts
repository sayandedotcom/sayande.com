...
export const projects: {
  ...
  },
  {
    ...
    bullets: [
      "LangGraph, LangChain, SambaNova (Llama 3.1 8B), Google Gemini 2.5 Flash, FastEmbed (Jina v2), Qdrant, BM25, PyPDFium, Streamlit",
      "Engineered a production-style Agentic RAG assistant replicating Uber's Genie internal AI agent architecture, which reportedly saved 21,000+ engineering hours annually by automating on-call policy and knowledge lookup.",
      "Orchestrated a multi-agent workflow in LangGraph as a stateful DAG (query rewriter → hybrid retriever → grounded generator), applying a cost-optimized model-routing strategy: a small 8B LLM for query rewriting and a frontier LLM for reasoning-heavy synthesis, cutting token spend without degrading answer quality.",
      "Implemented hybrid search in Qdrant, fusing dense semantic embeddings with sparse BM25 lexical vectors via multi-stage prefetch + re-ranking, improving recall on domain-specific and acronym-heavy queries over pure vector search.",
      "Designed an offline document-ingestion pipeline benchmarking PDF parsers for layout-faithful PDF→Markdown extraction, then chunking with LLM-generated custom metadata (source, page, keywords) to enable filtered retrieval and page-level citations that eliminate hallucinated answers.",
      "Shipped an interactive Streamlit chat interface with cached model warm-start and grounded-refusal guardrails that decline to answer when retrieved context is insufficient.",
    ],
    highlights: [
    ...
    engineering: [
      ...
      "https://www.uber.com/in/en/blog/genie-ubers-gen-ai-on-call-copilot/",
